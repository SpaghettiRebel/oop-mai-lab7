...
int main() {
    ...
    std::mutex &coutMtx = dungeon.coutMutex();

    dungeon.events().subscribe(std::make_shared<ConsoleLogger>(coutMtx));
    dungeon.events().subscribe(std::make_shared<FileLogger>("log.txt", coutMtx));
